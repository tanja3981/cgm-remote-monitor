...
function init (client, $) {
  ...
  _.forEach(careportal.allEventTypes, function each (event) {
    inputMatrix[event.val] = _.pick(event, ['bg', 'insulin', 'customDIA', 'carbs', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'reasons', 'targets']);
  });
  ...
  careportal.filterInputs = function filterInputs ( event ) {
    ...
    $('#customDIALabel').css('display',displayType(inputMatrix[eventType]['customDIA']));
    ...
    resetIfHidden(inputMatrix[eventType]['customDIA'], '#customDIA');
  ...
  function buildConfirmText(data) {
    ...
    pushIf(data.dia, translate('Custom DIA') + ': ' + data.dia);
