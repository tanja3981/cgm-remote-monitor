...
describe('IOB', function () {
    ...
    describe('from treatments', function () {
        ...
        it('should calculate IOB with custom DIA 2h using a 3 hour duration', function () {
            ...
            afterSomeTime.iob.should.be.lessThan(0.41);
            afterSomeTime.iob.should.be.greaterThan(0.4);

            var after2hDIA = iob.calcTotal(treatments, [], profile, time + (2 * 60 * 60 * 1000));

            after2hDIA.iob.should.equal(0);

            var after3hDIA = iob.calcTotal(treatments, [], profile, time + (3 * 60 * 60 * 1000));

            after3hDIA.iob.should.equal(0);

        });

        it('should calculate IOB with custom DIA 1h using a 1 hour duration', function () {
            ...
            afterSomeTime.iob.should.equal(0);

            var after2hDIA = iob.calcTotal(treatments, [], profile, time + (1.5 * 60 * 60 * 1000));
    ...
    describe('from devicestatus', function () {
        ...
        it('should fall back to treatments if openaps devicestatus is present but too stale', function () {
            var devicestatus = [_.merge(OPENAPS_DEVICESTATUS, {
